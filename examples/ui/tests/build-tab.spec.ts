...
test('Build tab - Convert address', async ({ page }) => {
    const chainReferenceInput: Locator = page.getByRole('textbox', { name: 'Search chain name...' });
    await page.getByRole('textbox', { name: 'Address @ Chain Reference' }).fill('0x1234567890AbcdEF1234567890aBcdef12345678');
    await chainReferenceInput.fill('1');
    await chainReferenceInput.blur();
