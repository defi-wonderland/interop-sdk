...
-   4210551: Accept protocol name strings in `createAggregator` providers
    ...
    `createAggregator({ providers })` now accepts the optional-config protocol names `"across"`, `"relay"`, `"bungee"` and `"lifi-intents"` alongside `CrossChainProvider` instances. Names are instantiated with default config via `createCrossChainProvider`, so `providers: ["across", "relay", "bungee", "lifi-intents"]` works without wiring each provider by hand. `oif` still requires an instance since its config is mandatory.
