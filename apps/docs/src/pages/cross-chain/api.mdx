...
| `sameAssetService` | `SameAssetService`     | No       | —                             | Resolves whether `buildQuote`'s input and output are the same asset, from a consumer-owned `assetId → chainId → address` map. When set, it replaces the default symbol/decimals/shared-provider heuristic; unknown pairings throw `DifferentAssetNotAllowed`. Addresses it resolves also survive discovery's metadata-conflict drops. See [Same-Asset Identity](#same-asset-identity). |
...
-   **discoverAssets**(options?: AssetDiscoveryOptions): Promise\<DiscoveredAssets\>
    ...
    Metadata from different providers is merged first-write-wins, and empty symbols are filled in from other sources. Tokens whose providers disagree on `symbol` or `decimals` are dropped with a warning — except addresses a configured `sameAssetService` resolves, which survive `symbol` disagreements; a `decimals` disagreement still drops them. See [Discovery conflicts](./advanced-usage#discovery-conflicts).
...
Opt-in resolver that decides whether two addresses are the same asset, from a map you control. `buildQuote` uses it to validate that input and output match; `discoverAssets` uses it to keep mapped addresses through metadata conflicts between providers. See [Advanced Usage](./advanced-usage#same-asset-transfers) for the full guide.
