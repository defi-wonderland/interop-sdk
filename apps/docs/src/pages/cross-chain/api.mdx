...
| `sameAssetService` | `SameAssetService`     | No       | —                             | Resolves whether `buildQuote`'s input and output are the same asset, from a consumer-owned `assetId → chainId → address` map. When set, it replaces the default symbol/decimals heuristic; unknown pairings throw `DifferentAssetNotAllowed`. See [Same-Asset Identity](#same-asset-identity).             |
