...
-   **discoverAssets**(options?: AssetDiscoveryOptions): Promise\<DiscoveredAssets\>
    ...
    Metadata from different providers is merged first-write-wins, and empty symbols are filled in from other sources. With a `sameAssetService` configured, tokens whose providers disagree on `symbol` or `decimals` are dropped with a warning instead — except addresses the service resolves, which are kept and listed under your asset id. See [Discovery conflicts](./advanced-usage#discovery-conflicts).
