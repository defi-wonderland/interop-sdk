---
title: "Cross-Chain: Frontend Integration"
description: Wire the cross-chain SDK into a React/Next.js app with wagmi v2 — a useCrossChainSwap hook covering quotes, approvals, and submission.
---
...
Everything on this page runs in the browser, which is the right place for open providers — each user calls the provider APIs from their own IP. Never put a provider `apiKey` in this code, though: anything in the bundle is public. For providers that require a key, point `baseUrl` at a proxy that injects it server-side — see [Open vs key-protected providers](/cross-chain/providers#open-vs-key-protected-providers).
    ...
    const execute = useCallback(
        async (request: QuoteRequest) => {
            ...
            try {
                ...
                // Track by orderId instead (see order-tracking.md).
                ...
                if (!submittedViaSignature && lastTxHash) {
                    ...
                    // standalone OrderTracker — see order-tracking.md.
                    setStatus("submitted");
                }
            } catch (err) {
...
-   [Order Tracking](/cross-chain/order-tracking) — detailed event reference and provider-specific notes
-   [Full Example](/cross-chain/example) — the equivalent Node.js script using `privateKeyToAccount`
