...
That heuristic leans on third-party token metadata and inherits the first-write-wins merge described in [Discovery conflicts](#discovery-conflicts): when providers disagree on a token's `symbol` or `decimals`, the check's outcome depends on which source answered first. If you'd rather decide asset identity yourself, pass a `sameAssetService` to `createAggregator`, built from a map you own — `assetId → chainId → address`:
...
Providers sometimes disagree about a token's metadata: one lists Base USDT as `USDT0` while another lists it as `USDT`, and some report an empty symbol. Without a `sameAssetService`, `discoverAssets` merges metadata first-write-wins and never drops a token. Empty symbols count as missing data and are filled in from whichever source has one, like `name` and `logoURI`.

Configuring a `sameAssetService` makes the merge strict. When sources disagree on a token's `symbol` or `decimals`, the token is dropped from the result and a warning is logged, except for addresses the service resolves: those survive symbol disagreements and are listed under your asset id, so Base USDT shows up as `USDT` no matter which provider answered first. A decimals disagreement drops the token even when the address is mapped; the map vouches for identity, not decimals.
