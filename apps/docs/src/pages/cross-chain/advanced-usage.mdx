---
title: "Cross-Chain: Advanced Usage"
description: Aggregator patterns for cross-chain transfers — multi-provider quote fetching, sorting strategies, timeouts, and built-in order tracking.
---
...
By default the aggregator returns every quote a provider hands back, including the contract addresses your users end up approving or calling. Pass a `spenderValidator` to `createAggregator` to check those addresses against a list you control. It does nothing unless you set it, so existing setups behave exactly as before.

`createSpenderValidator` builds the default validator from a per-chain allowlist keyed by chain ID:
...
const aggregator = createAggregator({
    providers: [createCrossChainProvider(PROTOCOLS.RELAY)],
    spenderValidator,
});
```

`getQuotes` drops any quote whose counterparty is not on the list and adds an `UntrustedSpender` entry to `response.errors`, so you can see what was filtered and why. `buildQuote` returns a single quote, so it throws `UntrustedSpender` instead.

### What gets checked

The validator reads counterparties from the quote payload, not from provider-supplied labels:

-   **Allowance spenders** — every `order.checks.allowances[].spender`.
-   **Approvals** — the decoded `approve` spender, but only when the step is tagged `category: "approval"` or a matching allowance check pins the same token and spender. An `approve`-shaped call to some other contract is checked as a plain transaction target instead, so a trusted address in the calldata can't wave through an untrusted `to`.
-   **Transaction targets** — the `transaction.to` of every other on-chain step.
-   **Signature counterparties** — the Permit2 `spender` and EIP-3009 `to` carried in EIP-712 signature steps. A signature whose counterparty can't be read is rejected.

Addresses are compared without regard to checksum casing. The check is fail-closed: a chain missing from the allowlist rejects all of its quotes, and `createSpenderValidator` throws if the allowlist is empty.

### Bringing your own registry

The SDK ships no built-in address list. You maintain `trustedSpenders`, or implement the `SpenderValidator` interface yourself to validate against your own registry:

```typescript
import type { SpenderValidator } from "@wonderland/interop-cross-chain";

const registryValidator: SpenderValidator = {
    findViolation(quote) {
        // return a SpenderViolation to reject the quote, or null to accept it
        return null;
    },
};

const aggregator = createAggregator({ providers, spenderValidator: registryValidator });
```

For the full API reference see [Spender Validation](./api#spender-validation).

## Same-asset transfers

`buildQuote` only builds same-asset transfers — the input and output must be the same asset on different chains. By default it infers this from discovery metadata: the two legs must share a symbol and decimals and be listed by a common provider. Cross-token swaps go through `getQuotes` instead, and `allowDangerousParameters: true` skips the check.

That heuristic leans on third-party token metadata, and tokens whose providers disagree on `symbol` or `decimals` are dropped from discovery entirely (see [Discovery conflicts](#discovery-conflicts)), making the check fail closed on disputed tokens. If you'd rather decide asset identity yourself, pass a `sameAssetService` to `createAggregator`, built from a map you own — `assetId → chainId → address`:
...
const aggregator = createAggregator({
    providers: [createCrossChainProvider(PROTOCOLS.ACROSS)],
    sameAssetService,
});
```

When set, `buildQuote` resolves both legs through the service and only proceeds when they map to the same `assetId`. It is fail-closed: if either address is unknown, the transfer is rejected with `DifferentAssetNotAllowed`. Addresses are matched without regard to checksum casing, and the native placeholders `0x0000…0000` and `0xEeee…EEeE` collapse to a single entry.

### Discovery conflicts

Providers sometimes disagree about a token's metadata: one lists Base USDT as `USDT0` while another lists it as `USDT`, and some report an empty symbol. Empty symbols count as missing data and are filled in from whichever source has one, like `name` and `logoURI`. Real disagreements are different — there is no way to tell which source is right, so when sources disagree on a token's `symbol` or `decimals`, the token is dropped from the result and a warning is logged.

A `sameAssetService` relaxes this for the addresses you map. The map already attests their identity (and `buildQuote` resolves it through the service, not the symbol), so a symbol disagreement at a mapped address keeps the token instead of dropping it — Base USDT stays discoverable no matter how each provider labels it, under the first symbol reported. A decimals disagreement drops the token even when the address is mapped; the map vouches for identity, not decimals.

### Bringing your own resolver

The SDK ships no token list and maintains none. You own the map, or implement the `SameAssetService` interface to resolve against your own registry:

```typescript
import type { SameAssetService } from "@wonderland/interop-cross-chain";

const registrySameAsset: SameAssetService = {
    resolve(chainId, address) {
        // return a stable asset id, or undefined to reject the pairing
        return undefined;
    },
};

const aggregator = createAggregator({ providers, sameAssetService: registrySameAsset });
```

For the full API reference see [Same-Asset Identity](./api#same-asset-identity).
...
When a [spender allowlist](#trusted-spender-allowlist) is configured, `buildQuote` also throws `UntrustedSpender`; the same error type shows up in the `errors` array of `getQuotes` rather than being thrown.
