...
description: Track cross-chain orders from initiation to completion via OIF OrderStatus events, with both onchain and offchain observation strategies.
---
...
Providers do not currently emit mixed-step orders (containing both transaction steps and signature steps) on their own. However, when an `approvalService` is wired in, the SDK can prepend approval `TransactionStep`s to signature-based orders, so consumers should still iterate `order.steps` in order and handle each step by its `kind`.
:::
